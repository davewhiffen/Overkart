            ...
            if (homingAccuracy > 0 && allKarts != null)
            {
                ...
                for (int i = 0; i < allKarts.Length; i++)
                {
                    if (allKarts[i] != castProps.castKart)
                    {
                        ...
                        {
                            first = false;
                            closeDist = (allKarts[i].transform.position - tr.position).sqrMagnitude;
                            closeAngle = Vector3.Dot((allKarts[i].transform.position - castProps.castPoint).normalized, moveDir);
                            ...
                            if (closeDist <= maxHomingDist * maxHomingDist && closeAngle >= minHomingAngle && lineOfSight)
                            {
                        ...
                        else
                        {
                            ...
                            if (curDist <= maxHomingDist * maxHomingDist && curAngle >= minHomingAngle && lineOfSight)
                            {
                                ...
                                {
                                    if (curAngle > closeAngle)
                                    {
                                        closeAngle = curAngle;
                                        SetHomingTarget(allKarts[i]);
                                    }
                                }
                                else if (curDist < closeDist)
                                {
                                    closeDist = curDist;
                                    SetHomingTarget(allKarts[i]);
                                }
